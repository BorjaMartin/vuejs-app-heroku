<template>
  <div class="container mx-auto" id="config">
      <b-modal
        id="modal-customer1"
...
export default {
  ...
  methods: {
    ...
    save(req) {
      ...
      if (Array.isArray(params)) {
        params.forEach((value) => {
          if (value.name) {
            if (
              !this.TrainersStatic.find((trainer) => trainer.name == value.name)
            ) {
    ...
    update() {
      ...
      clienteNoAsignado = this.CustomersStatic.sort((a, b) =>
        a.reputationImport < b.reputationImport
          ? 1
          : b.reputationImport < a.reputationImport
          ? -1
          : 0
      );
      ...
      var TrainersReputation = this.TrainersStatic.sort((a, b) =>
        a.assessment < b.assessment ? 1 : b.assessment < a.assessment ? -1 : 0
      );
          ...
          for (var i = 0; i < parseInt(trainerFind.maxCustomer); i++) {
            if (clienteNoAsignado.length > 0) {
              ...
              CustomersStaticUpdate.forEach((CustomersUpdate) => {
                if (
                  CustomersUpdate.name == CustomerName &&
                  CustomersUpdate.assign == false
                ) {
                  CustomersUpdate.assign = true;
                  trainerFind.customer.push(CustomersUpdate);

                  var resultSatisfaction = calcSatisfaction(
                    trainerFind.assessment,
                    CustomersUpdate.reputationImport
                  );
